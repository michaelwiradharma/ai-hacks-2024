...
def get_reply_topic(reply, topics):
    bedrock = AWSBedrock()
    prompt = f"Given the following reply, {reply} which topic among these topics: {topics} does it most correlate to? ONLY ANSWER WITH THE TOPIC NAMES AND NO ADDITIONAL WORDS, ONLY ANSWER WITH TOPIC NAMES ALREADY GIVEN. "
...
def get_post_report(sorted_replies, topics):
    bedrock = AWSBedrock()
    data_format = "{topic_n: summary, ...}"

    prompt = f"Given these topics: {topics} and its correlated replies: {sorted_replies}, write a summary about each topic. Follow this format: {data_format}."
    response = bedrock.get_reply(prompt)
    return response

# Extract topics from the post content and replies
def extract_topics(post, replies):
    bedrock = AWSBedrock()

    # combined_text = post + ' '.join(reply for reply in replies)
    data_format = "[topic1, topic2, ....]"

    prompt = f"From the following text, extract the main topics discussed. Keep the main topics less than 3 words Text: {post}"
    response = bedrock.get_reply(prompt)
...
print(extract_topics(post, replies))